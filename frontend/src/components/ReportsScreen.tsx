...
interface ProjectProgress { project_id: number; project_name: string; project_code: string; completion_pct: number; open_remarks: number }
interface WOBreakdown { status: string; count: number; avg_completion_pct: number }
interface OrgSummary { total_projects: number; active_projects: number; total_work_orders: number; completed_work_orders: number; open_remarks: number; critical_remarks: number; governance_holds: number }

const STATUS_COLOR: Record<string, string> = {
  DRAFT: '#94a3b8', PENDING_APPROVAL: '#f59e0b', APPROVED: '#3b82f6',
  IN_PROGRESS: '#6366f1', COMPLETED: '#10b981', CANCELLED: '#ef4444',
}
...
export function ReportsScreen() {
    ...
    const load = async () => {
      try {
        const [s, p, w] = await Promise.all([
          apiGet<OrgSummary>('/reporting/summary'),
          apiGet<{ items: ProjectProgress[] }>('/reporting/project-progress'),
          apiGet<{ breakdown: WOBreakdown[] }>('/reporting/work-orders'),
        ])
        setSummary(s)
        setProjects(p.items || [])
        setWoBreakdown(w.breakdown || [])
      } catch { /* offline */ }
      setLoading(false)
    }
    load()
  ...
  const handleIPCExport = async () => {
    ...
    try {
      const result = await apiPost<Blob>('/reporting/ipc', { format: 'csv', include_holds: true })
      const blob = result instanceof Blob ? result : new Blob([JSON.stringify(result)], { type: 'text/csv' })
      const url = URL.createObjectURL(blob)
      const a = document.createElement('a')
      a.href = url; a.download = `IPC-${new Date().toISOString().slice(0, 10)}.csv`; a.click()
    ...
    <div className="p-6 max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={handleIPCExport} disabled={exporting}
            ...
            <Download size={15} />{exporting ? 'Exporting…' : 'Export IPC'}
          </button>
        </div>
      </div>
      ...
      {summary && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          {[
            ...
            { label: 'Gov. Holds',       value: summary.governance_holds,  sub: summary.governance_holds > 0 ? 'Action needed' : 'All clear', color: summary.governance_holds > 0 ? 'orange' : 'emerald' },
                ...
                <div key={b.status}>
                  ...
                  <div className="h-2 bg-slate-100 rounded-full">
                    <div className="h-2 rounded-full transition-all"
                      style={{ width: `${totalWO > 0 ? (b.count / totalWO) * 100 : 0}%`, backgroundColor: STATUS_COLOR[b.status] || '#94a3b8' }} />
                  </div>
                </div>
              ))
            }
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="h-1.5 bg-slate-100 rounded-full">
                      <div className="h-1.5 rounded-full"
                        style={{ width: `${p.completion_pct}%`, backgroundColor: p.completion_pct >= 80 ? '#10b981' : p.completion_pct >= 40 ? '#6366f1' : '#f59e0b' }} />
                    </div>
                  </div>
                  <div className="shrink-0 text-right">
                    <div className="text-sm font-bold text-slate-700">{p.completion_pct.toFixed(0)}%</div>
                    {p.open_remarks > 0
                      ? <span className="text-xs text-amber-600 flex items-center gap-0.5"><AlertTriangle size={10} />{p.open_remarks}</span>
                      : <span className="text-xs text-emerald-600 flex items-center gap-0.5"><CheckCircle2 size={10} />clear</span>
                    }
                  </div>
                </div>
              ))
