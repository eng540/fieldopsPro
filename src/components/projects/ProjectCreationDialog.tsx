...
export interface ProjectCreationDialogProps {
  onCreated?: (project: any) => void | Promise<void>
  compact?: boolean
}

export function ProjectCreationDialog({ onCreated, compact = false }: ProjectCreationDialogProps) {
  ...
  const reset = () => { setName(''); setCode(''); setLocation('') }
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true)
    try {
      const project = await createProject({ name: name.trim(), code: code.trim(), location: location.trim() || null })
      toast({ title: 'تم إنشاء المشروع', description: `تم إنشاء «${name.trim()}» بنجاح` })
      setOpen(false)
      reset()
      await onCreated?.(project)
    } catch (error) {
      toast({ title: 'تعذر إنشاء المشروع', description: error instanceof Error ? error.message : 'حدث خطأ غير متوقع', variant: 'destructive' })
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button size={compact ? 'sm' : 'default'} className="gap-2 bg-emerald-600 hover:bg-emerald-700 text-white" dir="rtl"><Plus className="h-4 w-4" />مشروع جديد</Button>
      </DialogTrigger>
