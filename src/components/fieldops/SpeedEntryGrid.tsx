 ...
 * pipeline, version checks and offline queue instead of the legacy API/DB model.
 */
export function SpeedEntryGrid({ project, orgId, onRefresh }: Props) {
  ...
  const boqColumns = useMemo(() => {
    ...
    for (const unit of units) for (const item of unit.boqItems || []) if (!map.has(Number(item.id))) map.set(Number(item.id), item)
  ...
  const visibleCells = useMemo(() => {
    const cells: Cell[] = []
    for (const unit of units) for (const boq of boqColumns) if ((unit.boqItems || []).some((x: any) => Number(x.id) === Number(boq.id))) cells.push(getCell(unit, boq))
    return cells
  ...
  const saveCell = async (cell: Cell) => {
    ...
    try {
      const state = await getExecutionState(cell.unitId, cell.boqId)
      await submitProgressEvent({
        unitId: cell.unitId,
        boqItemId: cell.boqId,
        expectedVersion: Number(state?.state_version ?? 1),
        completionPct: cell.value,
        currentPct: Number(state?.completion_pct ?? cell.original),
      })
    } catch (error) {
      if (!isOnline()) {
        await queueProgressOffline(orgId, { unitId: String(cell.unitId), boqItemId: String(cell.boqId), completionPct: cell.value, reworkFlag: false, reworkReason: '' })
        return
      }
      throw error
    }
  ...
  return (
    <div className="space-y-3">
      <Alert><AlertTriangle className="h-4 w-4" /><AlertDescription>مصفوفة تشغيل ميداني: اختر الوحدات، عدّل البنود مباشرة أو طبّق نسبة جماعية، ثم احفظ عبر Event Pipeline. خفض الإنجاز يتطلب سبب إعادة تنفيذ.</AlertDescription></Alert>
      <Card><CardContent className="p-3">
        <div className="flex flex-col lg:flex-row gap-2 lg:items-center">
          <div className="relative flex-1"><Search className="absolute right-3 top-2.5 h-4 w-4 text-gray-400" /><Input className="pr-9" value={search} onChange={e => setSearch(e.target.value)} placeholder="بحث بالوحدة..." /></div>
          <div className="flex gap-2 items-center"><Input className="w-28" type="number" min="0" max="100" value={bulkValue} onChange={e => setBulkValue(e.target.value)} placeholder="% جماعي" /><Button variant="outline" onClick={applyBulk} disabled={!bulkValue || selectedUnits.size === 0}>تطبيق على المحدد</Button><Button onClick={saveAll} disabled={busy || !dirtyCells.length}>{busy ? <Loader2 className="h-4 w-4 ml-1 animate-spin" /> : <Save className="h-4 w-4 ml-1" />}حفظ {dirtyCells.length ? `(${dirtyCells.length})` : ''}</Button></div>
        </div>
        <div className="flex flex-wrap gap-2 mt-2 text-xs text-gray-500"><Badge variant="outline">{units.length} وحدة</Badge><Badge variant="outline">{boqColumns.length} بند</Badge><Badge variant="outline">{selectedUnits.size} محدد</Badge><Badge variant="outline">{dirtyCells.length} غير محفوظ</Badge></div>
      </CardContent></Card>
      <Card><CardHeader className="py-3"><CardTitle className="text-base">Unit × BOQ — Speed Entry</CardTitle></CardHeader><CardContent className="p-0">
        <div className="overflow-auto max-h-[68vh]"><table className="w-full text-xs min-w-[900px] border-collapse"><thead className="sticky top-0 z-20 bg-white"><tr className="border-b"><th className="sticky right-0 z-30 bg-white p-2 w-10"><Checkbox checked={units.length > 0 && selectedUnits.size === units.length} onCheckedChange={toggleAll} /></th><th className="sticky right-10 z-30 bg-white p-2 text-right min-w-[150px]">الوحدة</th>{boqColumns.map((boq: any) => <th key={boq.id} className="p-2 text-right min-w-[120px] border-r">{boq.code || boq.description}<div className="text-[10px] text-gray-400">{boq.unitOfMeasure || ''}</div></th>)}</tr></thead>
          <tbody>{units.map((unit: any) => <tr key={unit.id} className="border-b hover:bg-gray-50"><td className="sticky right-0 z-10 bg-white p-2"><Checkbox checked={selectedUnits.has(Number(unit.id))} onCheckedChange={() => toggleUnit(Number(unit.id))} /></td><td className="sticky right-10 z-10 bg-white p-2 font-medium">{unit.name}<div className="text-[10px] text-gray-400">{unit.code || `#${unit.id}`}</div></td>{boqColumns.map((boq: any) => { const exists = (unit.boqItems || []).some((x: any) => Number(x.id) === Number(boq.id)); if (!exists) return <td key={boq.id} className="p-2 border-r text-center text-gray-300">—</td>; const cell = getCell(unit, boq); const dirty = cell.value !== cell.original; return <td key={boq.id} className={`p-1 border-r ${dirty ? 'bg-amber-50' : ''}`}><div className="flex items-center gap-1"><Input className="h-8 w-20 text-center" type="number" min="0" max="100" value={cell.value} onChange={e => setCell(unit, boq, e.target.value)} />{cell.value >= 100 && <CheckCircle2 className="h-3.5 w-3.5 text-green-600" />}</div>{dirty && <div className="text-[9px] text-amber-700 text-center mt-0.5">{cell.original}% → {cell.value}%</div>}</td> })}</tr>)}</tbody>
        </table></div>
      </CardContent></Card>
