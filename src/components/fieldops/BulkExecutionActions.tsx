...
export function BulkExecutionActions({ units, selectedUnitIds, onApply, disabled }: Props) {
  ...
  const targetItems = useMemo(() => {
    ...
    if (group === 'ALL') return Array.from(new Map(items.map((i: any) => [Number(i.id), i])).values()).map((i: any) => Number(i.id))
  ...
  return <Card className="border-dashed"><CardContent className="p-3 space-y-3">
