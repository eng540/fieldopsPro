...
import {
  ...
  AlertCircle, Database, ArrowDownUp, Loader2
} from 'lucide-react'
  ...
  return (
    <div className="space-y-1">
      <div className="flex items-center gap-3 px-3 py-1.5 rounded-full bg-gray-100 text-xs">
        ...
        {isSyncing ? (
          <div className="flex items-center gap-1.5"><RefreshCw className="w-3.5 h-3.5 text-blue-600 animate-spin" /><span className="text-blue-700 font-medium">جاري المزامنة...</span></div>
        ) : hasPending ? (
          <div className="flex items-center gap-1.5"><UploadCloud className="w-3.5 h-3.5 text-amber-600" /><span className="text-amber-700 font-medium">{pendingCount} معلق</span></div>
        ) : (
          <div className="flex items-center gap-1.5"><CheckCircle2 className="w-3.5 h-3.5 text-emerald-600" /><span className="text-emerald-700">مزامنة</span></div>
        )}
        {stats && (
          <>
            <div className="w-px h-3 bg-gray-300" />
            <button onClick={() => setShowDetails(!showDetails)} className="flex items-center gap-1 text-gray-500 hover:text-gray-700"><Database className="w-3 h-3" /><span>{stats.projects}p • {stats.units}u • {stats.boqItems}b</span></button>
          </>
        )}
        <ProjectCreationDialog orgId={orgId} compact onCreated={onSync} />
        <Button variant="ghost" size="sm" onClick={onSync} disabled={isSyncing || !syncStatus.isOnline} className="h-6 px-2 text-xs ml-auto">
          {isSyncing ? <Loader2 className="w-3 h-3 animate-spin" /> : <ArrowDownUp className="w-3 h-3" />}
        </Button>
      </div>
