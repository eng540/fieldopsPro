...
import { Activity, Download, History, Loader2, RefreshCw, ShieldAlert } from 'lucide-react'
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card'
import { Button } from '@/components/ui/button'
import { Badge } from '@/components/ui/badge'
import { Input } from '@/components/ui/input'
import { getOrgSummary, getProjectProgress, getWorkOrderSummary, getExecutionEvents, downloadIPC } from '@/lib/api-client'
...
export function OperationsCenterScreen({ selectedProject }: { selectedProject: { id: string; name: string } | null }) {
  ...
  return <div className="space-y-6" dir="rtl">
    <div className="flex items-center justify-between"><div><h2 className="text-2xl font-bold">مركز التشغيل والتقارير</h2><p className="text-sm text-muted-foreground">مؤشرات التشغيل وسجل أحداث التنفيذ والتقارير.</p></div><div className="flex gap-2"><Button variant="outline" onClick={load}><RefreshCw className="ml-2 h-4 w-4" />تحديث</Button><Button onClick={() => downloadIPC('xlsx', true)}><Download className="ml-2 h-4 w-4" />IPC Excel</Button></div></div>
    ...
    {selectedProject && <Card><CardHeader><CardTitle>المشروع الحالي: {selectedProject.name}</CardTitle></CardHeader><CardContent><p className="text-sm text-muted-foreground">اختر المشروع من الشريط العلوي لمتابعة مؤشرات المشروع.</p></CardContent></Card>}
