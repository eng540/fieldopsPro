...
interface SelectedProject { id: string; name: string; code?: string; completionPct?: number; totalUnits?: number; units?: any[] }
...
export function OperationsCenterScreen({ selectedProject, remarks = [] }: { selectedProject: SelectedProject | null; remarks?: RemarkData[] }) {
  ...
  const load = useCallback(async (silent = false) => {
    ...
    try {
      ...
      setSummary(s.data); setProjects(p.data); setWorkOrders(w.data); setEvents(e.data?.items || []); setLastLoadedAt(new Date())
    } catch (err) { setError(err instanceof Error ? err.message : 'تعذر تحديث مركز التشغيل') }
    finally { setLoading(false); setRefreshing(false) }
  }, [])
  useEffect(() => { load() }, [load])

  const selectedProgress = useMemo(() => !selectedProject || !projects?.items ? null : projects.items.find((p: any) => String(p.project_id) === String(selectedProject.id)) || null, [projects, selectedProject])
  const units = useMemo(() => { const q = unitSearch.trim().toLowerCase(); return (selectedProject?.units || []).filter((u: any) => !q || `${u.name || ''} ${u.code || ''}`.toLowerCase().includes(q)) }, [selectedProject, unitSearch])
  ...
  const unitDetail = useMemo(() => {
    ...
    const items = selectedUnit.boqItems || []
    ...
    const boq = items.map((b: any) => ({ id: b.id, code: b.code ?? b.boqCode ?? b.boq_code ?? String(b.id), name: b.name ?? b.description ?? 'BOQ', progress: progressOf(b), status: b.status ?? '—', quality: b.quality ?? b.quality_pass ?? '—', planned: b.plannedQty ?? b.planned_qty, achieved: b.achievedQty ?? b.achieved_qty })).sort((a: any, b: any) => b.progress - a.progress)
