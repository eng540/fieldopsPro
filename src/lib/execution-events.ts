...
export interface ExecutionEventsResponse {
  succeeded: ExecutionEventResponse[]
  conflicts: Array<{ sync_uuid: string; expected_version: number; actual_version: number; current_state: Record<string, unknown>; resolution_options: string[] }>
  failed: Array<{ sync_uuid: string; error: string }>
}
...
export function createProgressIntent(input: { unitId: string | number; boqItemId: string | number; expectedVersion: number; deltaPct: number; rework?: boolean; reason?: string; transactionGroupId?: string }): ExecutionEventIntent {
  ...
  return {
    sync_uuid: uuidv4(), entity_type: 'BOQ_ITEM', entity_id: String(boqItemId), unit_id: unitId,
    event_class: 'PROGRESS', event_type: input.rework ? 'REWORK' : 'DELTA_ADD', metric_type: 'PERCENTAGE',
    value: { pct: input.deltaPct }, occurred_at: new Date().toISOString(), expected_version: input.expectedVersion,
