...
// Refresh can recover from the HttpOnly refresh cookie even when persisted
// Zustand tokens are missing/stale. A failed refresh always terminates in a
// deterministic unauthenticated state instead of an indeterminate UI state.
  ...
  persist(
    (set, get) => ({
      ...
      refreshAccessToken: async () => {
        ...
        // IMPORTANT: do not reject merely because Zustand lost the persisted
        // refresh token/session. The backend also accepts the HttpOnly cookie.
        // credentials: include below lets the browser recover the session.
        refreshInFlight = (async () => {
          try {
            const body = current?.refreshToken ? { refresh_token: current.refreshToken } : {}
            ...
            if (!res.ok || !data.access_token) {
              ...
              throw new Error(status === 401 ? 'SESSION_EXPIRED' : (data.detail || `فشل تحديث الجلسة (${status})`))
            }
            const expiresAt = Date.now() + ((data.expires_in || 900) * 1000)
            ...
            const raw = err?.message || 'فشل تحديث الجلسة'
            const message = raw === 'SESSION_EXPIRED'
              ? 'انتهت جلسة الدخول، يرجى تسجيل الدخول مرة أخرى'
              : (err?.name === 'AbortError' ? 'انتهت مهلة تحديث الجلسة' : raw)
            clearRefreshTimer()
            // Clear local auth state without issuing a remote logout. This is
            // essential when the refresh session itself has already expired.
      ...
      bootstrapAuth: async () => {
        ...
        const { tokens } = get()
        // Even without persisted tokens, try the HttpOnly cookie once. This
        // avoids falsely logging a user out after storage cleanup/expiry.
        if (!tokens?.sessionId && !tokens?.refreshToken) {
          try {
            await Promise.race([
              get().refreshAccessToken(),
              new Promise<never>((_, reject) => setTimeout(() => reject(new Error('انتهت مهلة تهيئة الجلسة')), REQUEST_TIMEOUT_MS + 2_000)),
            ])
            set({ isAuthenticated: true, isInitializing: false, error: null })
          } catch {
            await get().logout(false)
            set({ isAuthenticated: false, isInitializing: false, error: 'انتهت جلسة الدخول، يرجى تسجيل الدخول مرة أخرى' })
          }
          return
        }
