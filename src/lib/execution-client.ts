...
function token(): string | null { if (typeof window === 'undefined') return null; try { const raw = localStorage.getItem('fieldops-auth'); return raw ? JSON.parse(raw)?.state?.tokens?.accessToken || null : null } catch { return null } }
async function request<T>(path: string, options: RequestInit = {}): Promise<T> {
  const t = token(); const res = await fetch(`${API_BASE}${path}`, { ...options, headers: { 'Content-Type': 'application/json', ...(t ? { Authorization: `Bearer ${t}` } : {}), ...(options.headers || {}) } }); const body = await res.json().catch(() => ({})); if (!res.ok) throw new Error(body.detail || body.error || `HTTP ${res.status}`); return body as T
}
...
export async function listWorkOrders(projectId:string|number,page=1,pageSize=100){ return request<WorkOrderPage>(`/execution/work-orders?project_id=${projectId}&page=${page}&page_size=${pageSize}`) }
...
export async function submitProgressEvent(args:{unitId:number;boqItemId:number;expectedVersion:number;completionPct:number;currentPct:number;reworkReason?:string;occurredAt?:string}){
  const delta=args.completionPct-args.currentPct; const payload={sync_uuid:crypto.randomUUID(),entity_type:'BOQ_ITEM',entity_id:String(args.boqItemId),unit_id:args.unitId,event_class:'PROGRESS',event_type:delta<0?'REWORK':'DELTA_ADD',metric_type:'PERCENTAGE',value:{pct:Math.abs(delta)},occurred_at:args.occurredAt||new Date().toISOString(),expected_version:args.expectedVersion,reason:delta<0?args.reworkReason:undefined}; return request('/execution/events',{method:'POST',body:JSON.stringify({events:[payload]})})
}
export async function queueProgressOffline(orgId:string,item:{unitId:string;boqItemId:string;completionPct:number;reworkFlag:boolean;reworkReason:string}){
