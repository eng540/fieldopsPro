...
async function request<T>(path:string,options:RequestInit={}):Promise<T>{
  ...
  const body=await res.json().catch(()=>({})); if(!res.ok)throw new ApiError(res.status,body.detail||body.error||`HTTP ${res.status}`); return body as T
}
