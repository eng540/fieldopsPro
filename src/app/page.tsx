...
import { Building2, RefreshCw, LogOut, Settings, UploadCloud, WifiOff, Loader2 } from 'lucide-react'
...
export default function FieldOpsApp() {
  ...
  useEffect(() => { if (!isMounted || !isHydrated) return; void bootstrapAuth() }, [isMounted, isHydrated, bootstrapAuth])
  useEffect(() => { if (isMounted && isHydrated && !isInitializing && !isAuthenticated) router.replace('/login') }, [isMounted, isHydrated, isInitializing, isAuthenticated, router])
  ...
  const loadAllData = useCallback(async () => {
    ...
    try {
      ...
      if (a.data) { setProjects(a.data); setSelectedProject(current => { if (!a.data.length) return null; if (!current) return a.data[0]; return a.data.find(p => p.id === current.id) ?? a.data[0] }) }
  ...
  const allowedNavigation = getAllowedNavigation(user?.roles || [])
  const allowedNavIds = new Set(allowedNavigation.flatMap(group => group.items.map(item => item.id)))
  const navigate = (id: string) => {
    if (!allowedNavIds.has(id)) return
    if (id === 'project-configuration') {
  ...
  return <div className="min-h-screen flex flex-col bg-gray-50" dir="rtl">
    ...
    <div className="flex flex-1 overflow-hidden"><SectionSidebar activeTab={activeTab} onNavigate={navigate} userRoles={user?.roles || []}/>
      <main className="flex-1 overflow-y-auto pb-20 md:pb-0"><div className="p-4 md:p-6 max-w-7xl mx-auto"><WorkspaceBreadcrumb activeTab={activeTab} projectName={selectedProject?.name}/>
        <MobileSectionNav activeTab={activeTab} onNavigate={navigate} userRoles={user?.roles || []}/>
        ...
        {activeTab==='operations'&&wrap('مركز التشغيل', <OperationsCenterScreen selectedProject={selectedProject}/>)}
        {activeTab==='diary'&&wrap('السجل اليومي', <FieldDiaryScreen projectId={selectedProject?.id} projectName={selectedProject?.name} onChanged={loadAllData}/>)}
        {activeTab==='reports'&&wrap('التقارير', <LegacyReportsScreen/>)}
        ...
        {activeTab==='bulk-import'&&wrap('الاستيراد من Excel', <BulkImportScreen project={selectedProject} orgId={orgId} onRefresh={loadAllData}/>)}
        {activeTab==='work-orders'&&wrap('أوامر العمل', <WorkOrdersScreen project={selectedProject} orgId={orgId} onRefresh={loadAllData}/>) }
        {activeTab==='quality'&&wrap('الجودة', <QualityScreen project={selectedProject} remarks={remarks} orgId={orgId} onRefresh={loadAllData}/>)}
