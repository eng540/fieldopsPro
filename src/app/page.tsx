...
import { useOnlineStatus, useSyncEngine } from '@/lib/sync-hooks'
  ...
  const loadAllData = useCallback(async () => {
    try {
      const [projResult, userResult, remarkResult, auditResult, dictResult] = await Promise.all([
        getProjectsHybrid(orgId),
        getUsersHybrid(orgId),
        getRemarksHybrid(orgId),
        getAuditLogsHybrid(orgId),
        getDictionariesHybrid(orgId),
      ])

      if (projResult.data) {
        setProjects(projResult.data)
        if (projResult.data.length > 0 && !selectedProject) {
          setSelectedProject(projResult.data[0])
        }
      }
      ...
      <header className="sticky top-0 z-50 bg-white border-b border-gray-200 shadow-sm">
        <div className="flex items-center justify-between px-4 h-14">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-sm font-bold text-gray-900">FieldOps V4</h1>
              <p className="text-xs text-gray-500">منصة العمليات الميدانية</p>
            </div>
          </div>
