...
export default function FieldOpsApp() {
  ...
  const loadAllData = useCallback(async () => { try { const [a,b,c,d,e] = await Promise.all([getProjectsHybrid(orgId), getUsersHybrid(orgId), getRemarksHybrid(orgId), getAuditLogsHybrid(orgId), getDictionariesHybrid(orgId)]); if (a.data) { setProjects(a.data); if (a.data.length && !selectedProject) setSelectedProject(a.data[0]) }; if (b.data) setUsers(b.data); if (c.data) setRemarks(c.data); if (d.data) setAuditLogs(d.data); if (e.data) setDictionaries(e.data); const pending = await getPendingSyncItems(); setConflicts(pending.filter(i => i.status === 'CONFLICT' || i.status === 'FAILED')) } catch (err) { console.error('Load error:', err) } }, [orgId, selectedProject])
  ...
  return <div className="min-h-screen flex flex-col bg-gray-50" dir="rtl">
    ...
    <div className="flex flex-1 overflow-hidden"><aside className="hidden md:flex md:w-60 lg:w-64 flex-col bg-white border-l overflow-y-auto"><nav className="flex-1 px-3 py-4 space-y-1">{allowedNav.map(item=><button key={item.id} onClick={()=>navigate(item.id)} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm ${activeTab===item.id?'bg-emerald-50 text-emerald-700 border border-emerald-200':'text-gray-600 hover:bg-gray-50'}`}>{item.label}{item.badge&&<Badge variant="outline" className="text-[9px] mr-auto px-1 py-0 h-4">{item.badge}</Badge>}</button>)}</nav><div className="px-3 py-4 border-t"><div className="bg-amber-50 border border-amber-200 rounded-lg p-3"><div className="flex items-center gap-2 text-amber-800 text-xs font-medium mb-1"><UploadCloud className="w-3.5 h-3.5"/>طابور المزامنة</div><p className="text-xs text-amber-600">{isOnline?'متصل — جاهز للمزامنة':'غير متصل — محفوظ محلياً'}</p><Button variant="ghost" size="sm" className="text-xs h-6 mt-1" onClick={()=>navigate('mobile')}><Settings className="w-3 h-3 ml-1"/>إدارة الميدان</Button></div></div></aside>
      <main className="flex-1 overflow-y-auto"><div className="p-4 md:p-6 max-w-7xl mx-auto">
        {activeTab==='dashboard'&&<DashboardScreen projects={projects} selectedProject={selectedProject} users={users} remarks={remarks} auditLogs={auditLogs} dictionaries={dictionaries} onNavigate={navigate}/>} {activeTab==='operations'&&<OperationsCenterScreen selectedProject={selectedProject}/>} {activeTab==='diary'&&<FieldDiaryScreen projectId={selectedProject?.id} projectName={selectedProject?.name}/>} {activeTab==='reports'&&<LegacyReportsScreen/>} {activeTab==='mobile'&&<FieldMobileCenterScreen/>}
        {activeTab==='projects'&&<ProjectsScreen projects={projects} selectedProject={selectedProject} onSelectProject={setSelectedProject} orgId={orgId} onRefresh={loadAllData} dictionaries={dictionaries}/>} {activeTab==='speed-entry'&&<div className="space-y-4"><div className="flex items-center justify-between"><div><h2 className="text-2xl font-bold">مصفوفة الإدخال السريع</h2><p className="text-sm text-gray-500 mt-1">تحديث نسب الإنجاز — {selectedProject?.name||'اختر مشروعاً'}</p></div><Badge variant="outline"><WifiOff className="w-3 h-3 ml-1"/>أوفلاين</Badge></div><SpeedEntryGrid project={selectedProject} orgId={orgId} onRefresh={loadAllData}/></div>}
        {activeTab==='bulk-import'&&<BulkImportScreen project={selectedProject} orgId={orgId} onRefresh={loadAllData}/>} {activeTab==='work-orders'&&<WorkOrdersScreen project={selectedProject} orgId={orgId} onRefresh={loadAllData}/>} {activeTab==='quality'&&<QualityScreen project={selectedProject} remarks={remarks} orgId={orgId} onRefresh={loadAllData}/>} {activeTab==='governance'&&<GovernanceScreen orgId={orgId} onRefresh={loadAllData}/>} {activeTab==='users'&&<UsersScreen users={users} projects={projects} orgId={orgId} onRefresh={loadAllData}/>} {activeTab==='dictionary'&&<DictionaryScreen dictionaries={dictionaries} orgId={orgId} onRefresh={loadAllData}/>} {activeTab==='audit'&&<AuditScreen auditLogs={auditLogs} orgId={orgId} onRefresh={loadAllData}/>} {activeTab==='settings'&&<div className="space-y-6"><SettingsScreen orgId={orgId}/><ConflictResolutionPanel conflicts={conflicts} serverDataMap={Object.fromEntries(conflicts.filter(c=>c.serverData).map(c=>[c.operationUuid,JSON.parse(c.serverData!)]))} onResolve={()=>loadAllData()} onRefresh={loadAllData}/></div>}
