...
export default function ProjectConfigurationPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setUnits(canonical.units || unitResponse.items || [])
      setBoq(canonical.boq_items || [])
      setAssignments(canonical.assignments || [])
    } catch (e) {
      toast({ title: "تعذر تحميل إعدادات المشروع", description: e instanceof Error ? e.message : "خطأ غير متوقع", variant: "destructive" })
    } finally { setLoading(false) }
  }
  useEffect(() => { load() }, [orgId, projectId])

  const trades = useMemo(() => dictionaries.filter(d => (d.kind || d.category) === "trade" && d.isActive !== false).map(d => d.value || d.name || d.key).filter(Boolean) as string[], [dictionaries])
  const uoms = useMemo(() => dictionaries.filter(d => (d.kind || d.category) === "uom" && d.isActive !== false).map(d => d.value || d.name || d.key).filter(Boolean) as string[], [dictionaries])
  const unitTypes = useMemo(() => dictionaries.filter(d => (d.kind || d.category) === "unit_type" && d.isActive !== false).map(d => d.value || d.name || d.key).filter(Boolean) as string[], [dictionaries])
  ...
  const addDictionary = async () => {
    ...
    setSaving(true)
    try {
      await api("/projects/dictionaries", { method: "POST", body: JSON.stringify({ project_id: Number(projectId), kind: dictKind.trim().toLowerCase(), key: dictKey.trim() || dictValue.trim(), value: dictValue.trim(), sort_order: 0 }) })
      setDictKey(""); setDictValue(""); await load(); toast({ title: "تمت إضافة القيمة إلى القاموس" })
  ...
  const addUnit = async () => {
    ...
    setSaving(true)
    try {
      await api(`/projects/${projectId}/units`, { method: "POST", body: JSON.stringify({ name: unitName.trim(), code: unitCode.trim(), unit_type: unitType.trim(), floor: unitFloor.trim() || null, area_sqm: unitArea ? Number(unitArea) : null }) })
      setUnitName(""); setUnitCode(""); setUnitFloor(""); setUnitArea(""); await load(); toast({ title: "تمت إضافة الوحدة" })
    } catch (e) { toast({ title: "فشل إنشاء الوحدة", description: e instanceof Error ? e.message : "خطأ", variant: "destructive" }) }
    finally { setSaving(false) }
  }

  const addBoq = async () => {
    if (!trade.trim() || !description.trim() || !uom.trim() || !quantity) return
    setSaving(true)
    try {
      const item = await api<Boq>(`/projects/${projectId}/canonical-boq`, { method: "POST", body: JSON.stringify({ code: boqCode.trim() || null, trade: trade.trim(), description: description.trim(), quantity: Number(quantity), rate: 0, unit_of_measure: uom.trim() }) })
      setBoqCode(""); setDescription(""); setQuantity(""); await load(); setSelectedBoqId(String(item.id)); toast({ title: "تم إنشاء بند BOQ مركزي", description: "البند تعريف واحد للمشروع، وليس نسخة لكل وحدة." })
    } catch (e) { toast({ title: "فشل إنشاء بند BOQ", description: e instanceof Error ? e.message : "خطأ", variant: "destructive" }) }
    finally { setSaving(false) }
  }

  const applyBoq = async () => {
    if (!selectedBoqId || selectedUnitIds.size === 0) return
    setSaving(true)
    try {
      await api(`/projects/${projectId}/canonical-boq/${selectedBoqId}/apply`, { method: "POST", body: JSON.stringify({ unit_ids: Array.from(selectedUnitIds), planned_quantity: plannedPerUnit ? Number(plannedPerUnit) : 0 }) })
      await load(); toast({ title: "تم تطبيق البند على الوحدات", description: "تم إنشاء علاقة التطبيق وتهيئة حالة التنفيذ." })
    } catch (e) { toast({ title: "فشل تطبيق البند", description: e instanceof Error ? e.message : "خطأ", variant: "destructive" }) }
    finally { setSaving(false) }
  }
