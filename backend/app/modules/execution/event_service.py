...
async def _get_state(
    db: AsyncSession, *, org_id: int, unit_id: int, boq_item_id: int, for_update: bool = False
) -> UnitBoQProgress | None:
...
async def _cached_response(db: AsyncSession, sync_uuid: str) -> dict[str, Any] | None:
    result = await db.execute(
        select(EventSyncLog).where(EventSyncLog.operation_uuid == sync_uuid)
    )
    log = result.scalar_one_or_none()
    if log and log.response_payload is not None:
...
async def apply_event(
    ...
    *,
    ...
    """Apply exactly one event and materialize its resulting BOQ state atomically."""
    ...
    # A nested transaction makes the idempotency insert safe under concurrent retries.
    async with db.begin_nested():
        ...
        state = await _get_state(
            db, org_id=org_id, unit_id=unit_id, boq_item_id=boq_item_id, for_update=True
        )
        if state is None:
            if expected_version not in (None, 0, 1):
                raise HTTPException(
                    ...
                    detail={
                        ...
                        "message": "لا توجد حالة حالية مطابقة للنسخة المطلوبة.",
                        "conflict_details": {"expected_version": expected_version, "actual_version": None},
                    },
                )
        ...
        if event_type == EventType.STATUS_CHANGE:
            ...
            if value != old_status and value not in _ALLOWED_STATUS_TRANSITIONS.get(old_status, set()):
                if not (old_status == "COMPLETED" and value == "REWORK_REQUIRED" and authorized_role in {"PROJECT_MANAGER", "ORG_ADMIN", "SUPER_ADMIN"}):
                    raise HTTPException(
                        status_code=422,
                        detail=f"Invalid status transition: {old_status} -> {value}",
                    )
            state.status = value
            ...
            if metric_type == MetricType.PERCENTAGE:
                ...
                numeric = float(value)
                if event_type == EventType.DELTA_ADD:
                    new_value = current + numeric
                else:
                    new_value = numeric
                ...
                raise HTTPException(status_code=422, detail="This event type requires QUANTITY, PERCENTAGE, or FINANCIAL metric")

            if event_type == EventType.REWORK:
                if authorized_role not in {"PROJECT_MANAGER", "ORG_ADMIN", "SUPER_ADMIN"}:
