"""Project-scoped Excel import with preview, dry-run, and atomic commit."""
...
def _prepare_units(rows: list[tuple[Any, ...]], mapping: dict[str, int], existing_codes: set[str], org_id: int, project_id: int) -> tuple[list[dict[str, Any]], list[str]]:
    ...
    for row_no, row in enumerate(rows[1:], start=2):
        ...
        try:
            ...
            code = _text(row[mapping["code"]]).upper()
            ...
            if floor:
                try:
                    floor_value = int(float(floor))
                except (TypeError, ValueError) as exc:
                    raise ValueError(f"الصف {row_no}: «الطابق» يجب أن يكون رقمًا") from exc
            else:
                floor_value = None
...
async def preview_boq_excel(project_id: int, file: UploadFile = File(...), db: AsyncSession = Depends(get_db), current_user: dict = Depends(get_current_user)) -> dict[str, Any]:
...
async def import_boq_excel(project_id: int, file: UploadFile = File(...), dry_run: bool = Query(False), db: AsyncSession = Depends(get_db), current_user: dict = Depends(get_current_user)) -> dict[str, Any]:
    org_id = current_user["org_id"]
    project = (await db.execute(select(Project).where(Project.id == project_id, Project.org_id == org_id))).scalar_one_or_none()
    if not project:
        raise HTTPException(status_code=404, detail="Project not found")
...
async def preview_units_excel(project_id: int, file: UploadFile = File(...), db: AsyncSession = Depends(get_db), current_user: dict = Depends(get_current_user)) -> dict[str, Any]:
    org_id = current_user["org_id"]
    project = (await db.execute(select(Project).where(Project.id == project_id, Project.org_id == org_id))).scalar_one_or_none()
    if not project:
        raise HTTPException(status_code=404, detail="Project not found")
...
async def import_units_excel(project_id: int, file: UploadFile = File(...), dry_run: bool = Query(False), db: AsyncSession = Depends(get_db), current_user: dict = Depends(get_current_user)) -> dict[str, Any]:
    org_id = current_user["org_id"]
    project = (await db.execute(select(Project).where(Project.id == project_id, Project.org_id == org_id))).scalar_one_or_none()
    if not project:
        raise HTTPException(status_code=404, detail="Project not found")
