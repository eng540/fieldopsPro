...
async def list_diary(
    ...
    count = select(func.count()).select_from(FieldDiaryEntry).where(
    ...
    rows = (
        await db.execute(
            query.order_by(
                FieldDiaryEntry.diary_date.desc(), FieldDiaryEntry.created_at.desc()
            )
        )
    ).scalars().all()
...
async def create_diary(
    ...
    project = (
        await db.execute(
            select(Project).where(Project.id == data.project_id, Project.org_id == org_id)
        )
    ).scalar_one_or_none()
    if not project:
    ...
    entry = FieldDiaryEntry(
        id=str(uuid4()), org_id=org_id, created_by=user_id, **data.model_dump()
    )
    db.add(entry)
    await db.flush()
...
async def get_diary(
    ...
    row = (
        await db.execute(
            select(FieldDiaryEntry).where(
                FieldDiaryEntry.id == entry_id,
                FieldDiaryEntry.org_id == current_user["org_id"],
            )
        )
    ).scalar_one_or_none()
    if not row:
