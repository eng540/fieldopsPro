...
def upgrade() -> None:
    ...
    # One-to-one migration of existing current-state rows into INITIAL_STATE events.
    # Each legacy row gets its own event/sync UUID pair and its complete legacy state
    # is preserved in event.new_value.
    op.execute(
        """
        DO $$
        DECLARE
    ...
    # RLS is strict for application sessions: missing org context means no access.
    ...
    # Database-level WORM guard for the business event log.
    op.execute(
        """
    ...
    # The application role may write events, but cannot UPDATE/DELETE them.
